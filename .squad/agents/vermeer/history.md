...
- **2026-01-XX (vermeer-fieldbattle-css-fix):** Fixed missing CSS for FieldBattle and TroopTransfer notification panels. Both `FieldBattleInvitePanel` and `TroopTransferReceivedPanel` used `notification-panel`, `notification-panel__message`, `notification-panel__countdown`, and `notification-panel__actions` CSS classes, but NO stylesheet defined them anywhere. Panels were rendering but completely invisible (no layout, no styling, no animation). Created `notification-panel.css` with full glassmorphism styling matching the ability-card design: top-centered position, slide-down animation, backdrop blur, proper spacing, button layout, variant modifiers for troop-transfer (blue glow) and field-battle (red glow). Imported the stylesheet in both panel components (`import '../../../styles/notification-panel.css'`) following the existing pattern from `AbilityCard.tsx`. The SignalR event registration was already correct: `connection.on('FieldBattleInvite', ...)` on line 151 of `useSignalR.ts`, `onFieldBattleInvite` handler on line 665 of `useSignalRHandlers.ts` calls `setFieldBattleInvite(data)` and pushes a toast, `FieldBattleInvitePanel` reads `fieldBattleInvite` from notification store on line 26 and renders when non-null. Build: `npm run lint && npm run build` — 0 errors, 302 modules, clean. **Root cause:** The CSS was never written when these panels were first implemented; the markup was there but with zero styling, so they rendered but were invisible.
