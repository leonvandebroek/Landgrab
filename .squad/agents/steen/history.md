...
- 2026-03-23 (visibility/compass/Scout validation): Landgrab MCP again returned `Not connected` for all tool calls. Performed code-level validation of three shipped fixes: (1) instant tile visibility — `isLocallyVisible()` in `localVisibility.ts` + `useMemo` in `HexTile.tsx` provide zero-round-trip client-side visibility derivation on player move; code correct. (2) compass stability — perpetual rAF loop in `lerpBearing` fixed via convergence check `Math.abs(diff) < 0.3` that terminates the loop and sets `bearingRafRef.current = 0`; restarted only when heading changes. (3) Scout ability gating — `beacon` and `shareIntel` pushed to `abilityButtons` only inside `if (rolesEnabled && player?.role === 'Scout')` block in `PlayerHUD.tsx`; Commander sees only tacticalStrike + commandoRaid. All three fixes look correct at code level; live browser validation blocked by MCP connectivity.
